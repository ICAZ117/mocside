  ...
  methods: {
      ...
      if (lab != undefined && JSON.stringify(lab) != JSON.stringify({})) {
        if (!course.isAdded) {
          ...
          if (this.copies.length > 1) {
            ...
            var flag = confirm(
              "This is the last copy of this assignment, are you sure you want to delete it forever"
            );
            if (flag) {
    ...
    addToAll() {
      for (let i = 0; i < this.courses.length; i++) {
        if (
          this.courses[i].currentLab != undefined &&
          JSON.stringify(this.courses[i].currentLab) != JSON.stringify({})
        ) {
